...
def tf_idf_func_gender():
    ...
    stop_list = get_stop_words('italian')

    # we use a dummy function as tokenizer
    tfidf_vec = TfidfVectorizer(preprocessor = preProcess, tokenizer = identity, stop_words = stop_list, ngram_range=(2, 2))

    count_vec = TfidfVectorizer(analyzer='char', ngram_range=(2, 7))
...
def SVM_Linear(trainDoc, trainClass, testDoc, testClass, use_gender):
    ...
    if use_gender:
        ...
        print("For Italian We don't have any data for Age")
        # vec = tf_idf_func_age()
...
def SVM_baseline(trainDoc, trainClass, testDoc, testClass, use_gender):
    ...
    print("\n########### Baseline SVM Classifier For ", classType, " in Italian ###########\n")

    # Call to function(s) to show the results for development set ^_^
    print("\n==>Measures on the Test set:\n")
    title = "Baseline - Italian ("+classType+")"
    calculate_measures(classifier, testClass, testGuess, title)
...
def main():
    ...
    # combining the guess gender with author id to creat the truth.txt file in the test folder
    out = []
    for id1, gen in zip(id_gen, guess_gen):
        out_line = id1+":::"+gen+":::XX-XX"
        out.append(out_line)

    # creating the truth.txt file in the test/dutch folder
