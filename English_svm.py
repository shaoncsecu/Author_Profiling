...
def process(x):
    ...
    # This has to be done this way because it's a list of list
...
def tf_idf_func_gender():
    ...
    tfidf_vec = TfidfVectorizer(preprocessor = preProcess, tokenizer = identity, stop_words = 'english', ngram_range=(1, 2))
...
def tf_idf_func_age():
    ...
    tfidf_vec_word = TfidfVectorizer(stop_words='english', preprocessor = preProcess,
...
def SVM_baseline(trainDoc, trainClass, testDoc, testClass, use_gender):
    ...
    print("\n########### Baseline SVM Classifier For ", classType, " in English ###########\n")

    # Call to function(s) to show the results for development set ^_^
    print("\n==>Measures on the Test set:\n")
    title = "Baseline - English ("+classType+")"
    calculate_measures(classifier, testClass, testGuess, title)
